...
const createId = ({ node, browser }, val, boy) => new Promise((resolve, reject) => {
  if (boy.get(node) && boy.get(node).virtual) {
    resolve({ node, browser, id: { val: boy.get(node).id } })
  } else if ((val && val.virtual) || exclude(node)) {
    if (exclude(node)) {
      node = 'EXCLUDE-' + node
    }
        ...
        fs.readFile(join(pkgPath, 'package.json'), (err, data) => {
          if (err) {
            ...
            try {
              ...
              parseJSONError(data.toString(), err)
              err.file = join(pkgPath, 'package.json')
              reject(err)
            }
          }
        })
...
const resolvePaths = (require, from) => new Promise((resolve, reject) => {
  const basedir = from.vritual
    ? from.vritual === true ? false : from.vritual
    : dirname(from.key)
    ...
    if (err) {
      ...
      if (exists && exists.virtual) {
        ...
        for (let i = 0; i < lines.length; i++) {
          cnt += (lines[i].length)
          if (cnt > from.code.compute().indexOf(require)) {
            err.message = `Cannot find module "${require}" (${i + 1}:0)`
            break
          }
        }
        reject(err)
      ...
      return new Promise((resolve, reject) => {
        if (real && this.get(real)) {
          ...
          const result = real || builtin(require)
            ? createId({ node: real || require }, val, this)
            : resolvePaths(require, from)
              .then(paths => createId(paths, val, this))
          ...
          result.then(({ node, browser, id, version }) => {
            if (!this.get(node)) {
              ...
              if (node.indexOf('EXCLUDE-') !== 0 && !val.virtual) {
              ...
              this.set({ [node]: { require } })
              if (browser && browser !== node) {
                this.set({ [browser]: { require } })
              }
            }
        ...
        on: {
          data: {
            parse (val, stamp, code) {
              if (val !== null) {
                console.log('fire code?', code.parent().key)
