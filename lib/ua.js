...
module.exports = ua => {
  ...
  const final = finalResults(results, clean)
  console.log('W00000t', final)
  // final post parse (fast checks etc , create function perhaps for the check -- max speed)
  // pass ua get result
...
const finalResults = (results, clean) => {
  // console.log(results.map(val => val))
  ...
  for (let i = 0, len = results.length; i < len; i++) {
    for (let j = i + 1; j < len; j++) {
      if (isEqual(results[i]._match, results[j]._match)) {
        results.splice(j, 1)
        len--
        i--
        j--
  ...
  for (let i = 0, len = results.length; i < len; i++) {
    ...
    matchIt(match, final, results[i], list)
  }
