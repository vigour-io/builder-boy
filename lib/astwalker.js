            ...
            for (let i in imports.exports) {
              if (i !== 'default' && i !== '*') {
                if (imports.isExternal) {
                  imports.vars[imports.exports[i][0]] = `${imports.id}.${imports.exports[i][1]}`
                } else {
      ...
      for (let i = 0, len = computed.length; i < len; i++) {
        ...
        if (imports && i === imports.start) {
          ...
          if (i === insertId[j]) {
            ...
            // if (replaceImports[k + 2] && replaceImports[k + 2].isExternal) {
              // console.log('isExternal', replaceImports[k + 2].isExternal)
            // }
            result += replaceImports[k + 2].vars[replaceImports[k + 1]]
            i += replaceImports[k + 1].length - 1
            k += 3
            console.log('go go go go -----> ????', replaceImports[k + 1], replaceImports[k + 2])
          } else {
            result += computed[i]
          }
        }
