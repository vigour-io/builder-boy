...
const setStore = (node, replaceImports, insertId, store) => {
  if (
    store[node.name] &&
  ...
  walk(node => {
    ...
    if (node.type === 'ExportDefaultDeclaration') {
      ...
      if (node.declaration.type === 'Literal') {
        exports.literal = node.declaration.raw
      } else {
        if (node.declaration.type === 'Identifier') {
          exports.identifier = node.declaration.name
        }
      }
      ...
      node.specifiers.forEach(node => {
        ...
        console.log('WTF', node.local.name)
        store[node.local.name] = imports
      })
      insertImports.push(imports)
    } else {
      identifier(node, replaceImports, insertId, store)
    }
  })
    ...
    for (let i = 0; i < computed.length; i++) {
      ...
      if (imports && i === imports.start) {
        ...
        if (exports.type === 'default') {
          if (exports.literal) {
            result += `const ${id} = ${exports.literal}`
            i += (end - start)
          } else if (exports.identifier) {
            if (insertId[j] < end && insertId[j] > start) {
              result += `const ${id} = ${id + '_' + exports.identifier}`
              i += (end - start)
              j++
            } else if (replaceImports[k] < end && replaceImports[k] > start) {
              result += `const ${id} = ${replaceImports[k + 2].vars[replaceImports[k + 1]]}`
              i += (end - start)
              k += 3
            }
          }
