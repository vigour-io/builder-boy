  ...
  return new Promise((resolve, reject) => {
    ...
    const parse = node => {
      if (node.type === 'ExportNamedDeclaration') {
        ...
        console.log('ok module.exports', node.parent.parent.right)
        const exports = {
      ...
      if (node.name === 'require' && node.parent.type === 'CallExpression') {
        // console.log('lulllz', node.parent.arguments[0].value)
        const p = node.parent.parent.parent
        console.log(node.parent.parent.parent, node.parent.parent.id.name, node.parent.parent.parent.end)
        const imports = {
          type: 'imports',
