    ...
    if (!hasLocalVar(node)) {
      ...
      if (store[node.name] !== true && store[node.name].type === 'imports') {
        if (
          ...
          if (!isImport(node)) {
            console.log('ADD IMPORT', node.name, node.start)
...
module.exports = code => {
  return new Promise((resolve, reject) => {
    ...
    Promise.all(insertImports.map(val => prepImports(val, file))).then(results => {
      ...
      if (replaceImports.length) {
        console.log('replaceImports', replaceImports)
        console.log(insertImports)
      }
      // console.log('\n\n', replaceImports.map(val => val.name + ':' + val.start))
      ...
      // result += '// ' + replaceImports.map(val => val.name + ':' + val.start) + '\n'
