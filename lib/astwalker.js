...
module.exports = code => {
  return new Promise((resolve, reject) => {
    ...
    const parse = node => {
      ...
      if (node.name === 'require' && node.parent.type === 'CallExpression') {
        if (node.parent.parent.id) {
          ...
          showcode(p.start, p.end, computed)

          if (insertId[insertId.length - 1] >= p.start && insertId[insertId.length - 1] < p.end) {
            insertId.pop()
          }
    ...
    Promise.all(insertImports.map(val => prepImports(val, file, computed))).then(dependencies => {
      ...
      console.log(replaceImports.map(val => val.name + ':' + val.start))
