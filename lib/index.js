  ...
  const finish = (result, inline, field = 'result') => {
    ...
    const compile = module => {
      ...
      if (
        ...
        if (inline && module.file.replaceInline) {
          // can be better with inverted sourcemaps (map lines back)
          module.file.replaceInline.forEach(val => {
            str = str.replace(val.original, val.val)
          })
          console.log('go go go', module.file.replaceInline)
        }
