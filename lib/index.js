...
const moreBuild = (real, rdy) => {
  ...
  build(real, file).then(result => {
    ...
    inlineBrowser = `(function (global) { ${inlineBrowser} })(window)`
...
const build = (name, root) => {
  const finish = (result, excludeExternal = true) => {
    var str = ''
    const compile = module => {
      if (!module) return ''
      if (excludeExternal && module.external) {
        str = str + `\n // FILE: ${module.name}\n` + module.external
      } else {
  ...
  return Promise.all([
    ...
    buildBrowser(name, root).then(result => {
      var browserNode = finish(result)
      var inlineBrowser = finish(result, true)
