...
const moreBuild = (real, rdy) => {
  ...
  build(real, file).then(([ node, { browserNode, inlineBrowser } ]) => {
    ...
    inlineBrowser.code = `(function (global, exports, module) { ${inlineBrowser.code} })(window, {}, {})`
...
const build = (name, root) => {
  ...
  return Promise.all([
    ...
    buildBrowser(name, root).then(result => {
      ...
      const inlineBrowser = finish(JSON.parse(JSON.stringify(result)), true)
