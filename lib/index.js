...
const moreBuild = (real, rdy) => {
  ...
  build(real, file).then(result => {
    ...
    if (!result[1]) {
      result[1] = { browserNode: '', inlineBrowser: '' }
    }
...
const buildFactory = (depsKey = 'dependencies', includeExternal) => {
...
const buildNormal = buildFactory('dependencies', true)
const buildBrowser = buildFactory('browser', true)
