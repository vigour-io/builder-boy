...
const moreBuild = (real, rdy, opt = {}) => {
  ...
  build(real, file, opt).then(([ node, { browserNode = { code: '' }, inlineBrowser = { code: '' } } ]) => {
    ...
    if (inlineBrowser.env) {
      console.log(
        `   ${chalk.green('process boys')}`, '\n    ',
        Object.keys(inlineBrowser.env).map(val => `${val}: ${inlineBrowser.env[val]}`).join('\n     ')
      )
    }
    ...
    inlineBrowser.code = `(function (global, process) { \n${inlineBrowser.code};\n })(window, {})`
...
const build = (name, root, opt) => {
  const finish = (result, inline, field = 'result') => {
    ...
    const compile = module => {
      ...
      if (module.name === '/Users/jimdebeer/Desktop/brisky/play/test/ploy/index.js') {
        console.log(chalk.red('hit it?'), module.stamp === stamp)
      }
      ...
      if (module.externalName) {
        console.log('EXTERNAL...', module.externalName)
