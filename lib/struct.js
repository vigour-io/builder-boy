...
const update = ({ dependencies, result, code, any, hasDefault, es5, uaResult }) => {
  ...
  dependencies.forEach(dep => {
    ...
    // if (file.key.indexOf('builder-boy/lib/resolve/index.js') !== -1) {
    //   console.log(chalk.red('SET -- warning: (sparks a lot of weirdness'), file.key)
    //   file.set({ result: `console.log("resolve included...");`, hasDefault: false, any: false })
    //   return
    // }
  ...
  if (uaResult) {
    console.log(JSON.stringify(uaResult, false, 2))
  }
  file.set({ result, es5, any, hasDefault, uaResult })
}
