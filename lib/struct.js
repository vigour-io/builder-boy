...
const update = ({ dependencies, result, code, any, hasDefault }) => {
          ...
          [node]: {
            ...
            on: {
              data: {
                deps: (val, stamp, t) => {
                  if (file.result) {
                    console.log('----->', stamp, t.key.replace('Users/jimdebeer/Desktop/brisky/', ''), file.key.replace('Users/jimdebeer/Desktop/brisky/', ''))
                    file.result.emit('data', val, stamp)
                  }
                }
              }
  ...
  console.log('SET', stamp, file.key.replace('Users/jimdebeer/Desktop/brisky/', ''))
  file.set({ result, any, hasDefault }, stamp)
  bs.close()
}
