...
const simplify = test => {
  if (test.$or) {
    // Tries taking repeating comparisons out of or blocks
    const andPart = JSON.parse(JSON.stringify(test))
    delete andPart.$or
    let i = test.$or.length
    while (i--) {
      if (!test.$or || !test.$or[i]) {
        break
      }
      let keys = {}
      let out = []
      ...
      while (j--) {
        ...
        if (JSON.stringify(orClone) === JSON.stringify(andPart)) {
          test.$or[i] = []
    ...
    for (let key in test) {
      if (key !== '$or') {
        ...
        while (i--) {
          ...
          let j = test.$or[i].length
          while (j--) {
            const and = JSON.parse(JSON.stringify(test.$or[i][j]))
            before += Object.keys(and).length
            for (var operator in test[key]) {
              if (!and[key]) {
                and[key] = {}
              }
