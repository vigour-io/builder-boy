    ...
    if (key !== '$or' && key !== '$bin') {
      ...
      while (i--) {
        ...
        if (~found && !andFrom[key].$nin) {
          ...
          if (andPatch(newAnd[key], invertOperator(operator), impossible[i][key][operator]) === -9) {
            keys.splice(found, 1)
            let key2 = keys[0]
            if (andTo[key2] && !andTo[key2].$nin) {
              let operator = Object.keys(impossible[i][key2])[0]
              let res = andPatch(andTo[key2], invertOperator(operator), impossible[i][key2][operator])
              if (res === -9) {
                andKill(andTo)
                return
              }
