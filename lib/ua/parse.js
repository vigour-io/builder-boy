...
const impossible = [
  { browser: { $in: ['ie', 'edge'] }, device: { $in: ['cast', 'tv', 'firetv'] } },
  { browser: { $in: ['ie', 'edge'] }, platform: { $in: ['android', 'ios'] } },
  { browser: { $in: ['ie', 'edge'] }, prefix: { $in: ['moz'] } },
  { browser: { $in: ['ie', 'edge'] }, webview: { $in: ['ploy-native'] } },
  { browser: { $in: ['ie', 'edge'] }, prefix: { ['<']: 10 } },

  { browser: { $in: ['chrome'] }, prefix: { $in: ['ms', 'moz'] } },
  { browser: { $in: ['chrome'] }, prefix: { ['<']: 30 } },

  { browser: { $in: ['safari'] }, prefix: { $in: ['ms', 'moz'] } },

  { browser: { $in: ['firefox'] }, device: { $in: ['cast'] } },
  { browser: { $in: ['firefox'] }, prefix: { $in: ['ms', 'webkit'] } },
  { browser: { $in: ['firefox'] }, webview: { $in: ['ploy-native'] } },
  { browser: { $in: ['firefox'] }, prefix: { ['<']: 20 } },

  { device: { $in: ['bot'] }, platform: { $in: ['android', 'ios'] } },
  { device: { $in: ['bot'] }, webview: { $in: ['ploy-native'] } },

  { device: { $in: ['cast'] }, webview: { $in: ['ploy-native'] } },
  ...
  for (var key in andFrom) {
    if (key !== '$or' && key !== '$bin') {
      ...
      while (i--) {
        ...
        if (~found && !andFrom[key].$nin) {
          let newAnd = { [key]: andFrom[key] }
          let before = JSON.stringify(newAnd[key])
          let operator = Object.keys(impossible[i][key])[0]
          let res = andPatch(newAnd[key], invertOperator(operator), impossible[i][key][operator])
          sortAnd(newAnd)
          if (JSON.stringify(newAnd[key]) !== before) {
            keys.splice(found, 1)
            let key2 = keys[0]
            if (andTo[key2] && !andTo[key2].$nin) {
              let newAnd2 = { [key2]: andTo[key2] }
              let operator = Object.keys(impossible[i][key2])[0]
              if (andPatch(newAnd2[key2], invertOperator(operator), impossible[i][key2][operator]) === -9) {
                if (res === -9) {
                  andKill(andTo)
                  return
                } else {
                  andFrom[key] = newAnd[key]
                }
              }
            }
          }
  ...
  for (var key in test) {
    if (key !== '$or' && key !== '$bin') {
      for (var operator in test[key]) {
        or.push({ [key]: { [invertOperator(operator)]: test[key][operator] } })
      }
    }
  }
...
// Tries distributing and's over or's
const simplify = test => {
  if (test.$or) {
    for (let key in test) {
      if (key !== '$or') {
        ...
        while (i--) {
          ...
          // Check if it worths distributing
          if (before >= after) {
            delete test[key]
            var gotFalse = false
            ...
            var k = newOr.length
            while (k--) {
              if (newOr[k].hasOwnProperty('$bin') && !newOr[k].$bin) {
                gotFalse = true
                newOr.splice(k, 1)
              }
            }
