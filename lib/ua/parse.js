...
const andPatch = (and, operator, val) => {
  if (operator === '$in' && and['$nin']) {
  ...
  } else if ((operator === '==' || operator === '===' || operator === '$in') && (and['>'] || and['>='] || and['<'] || and['<='])) {
    const list = [].concat(val)
    let i = list.length
    while (i--) {
      if ((and['>'] && and['>'] >= list[i]) || (and['>='] && and['>='] > list[i]) || (and['<'] && and['<'] <= list[i]) || (and['<='] && and['<='] < list[i])) {
        list.splice(i, 1)
      }
    }
    if (list.length) {
    ...
    and[operator] = String(val)
  } else if ((operator === '!=' || operator === '!==' || operator === '$nin') && (and['>'] || and['>='] || and['<'] || and['<='])) {
    const list = [].concat(val)
    let i = list.length
    while (i--) {
      if ((and['>'] && and['>'] <= list[i]) || (and['>='] && and['>='] < list[i]) || (and['<'] && and['<'] >= list[i]) || (and['<='] && and['<='] > list[i])) {
...
const invert = (test, inverted) => {
  ...
  if (or.length > 1) {
    ...
    if (!or[0].hasOwnProperty('$bin') || or[0].$bin === false) {
