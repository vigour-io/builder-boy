...
const impossible = [
  ...
  { browser: { $in: ['ie', 'edge'] }, version: { '<': 10 } },

  { browser: { $in: ['chrome'] }, prefix: { $in: ['ms', 'moz'] } },
  // { browser: { $in: ['chrome'] }, version: { '<': 30 } },
      ...
      if (!andFrom[key].$nin) {
        ...
        while (i--) {
          ...
          if (~found) {
            ...
            if (andPatch(and, invertOperator(operator), impossible[i][key][operator]) === -9) {
              let key2 = keys[(found + 1) % 2]
              if (andTo[key2] && !andTo[key2].$nin) {
                let operator = Object.keys(impossible[i][key2])[0]
                if (andPatch(andTo[key2], invertOperator(operator), impossible[i][key2][operator]) === -9) {
                  andKill(andTo)
                  return
                }
              }
