    ...
    if (key !== '$or' && key !== '$bin') {
      ...
      while (i--) {
        ...
        if (~found && !andFrom[key].$nin) {
          ...
          let before = JSON.stringify(newAnd)
          let operator = Object.keys(impossible[i][key])[0]
          let res = andPatch(newAnd, invertOperator(operator), impossible[i][key][operator])
          sortAnd(newAnd)
          if (JSON.stringify(newAnd) !== before) {
            keys.splice(found, 1)
            let key2 = keys[0]
            if (andTo[key2] && !andTo[key2].$nin) {
              let newAnd2 = { [key2]: andTo[key2] }
              let operator = Object.keys(impossible[i][key2])[0]
              if (andPatch(newAnd2, invertOperator(operator), impossible[i][key2][operator]) === -9) {
                if (res === -9) {
                  andKill(andTo)
                  return
                } else {
