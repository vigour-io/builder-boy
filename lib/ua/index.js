...
module.exports = (node) => {
  ...
  val.forEach(val => {
    val.id = hash(val.ua)
    builds[val.id] = val.ua
  })

  // node.code.replace(file.code, `var ${file.val}=${val.ua}`)
  const result = {
    val,
    builds,
    replace: file.val,
...
const generateCondition = (val, wrap) => {
  const and = []
  if (val.$or) {
    let i = val.$or.length
    while (i--) {
      ...
      while (j--) {
        or.push(generateCondition(val.$or[i][j], true))
      }
      and.push(`(${or.join(' || ')})`)
    }
  }
  ...
  const joined = and.join(' && ')
  return wrap && and.length > 1 ? `(${joined})` : joined
}
