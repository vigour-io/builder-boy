...
const checkInlineOption = (inline, packagePath) => {
  let result = false
  inline.forEach(i => {
    const type = typeof i
    if (!result) {
      if (type === 'string') {
        result = i === packagePath
...
const inline = (file, result, type, opts) => type === 'inline' || file.virtual || (
  opts.inline && checkInlineOption(opts.inline, file.external)
)
...
exports.compile = (file, result, type, opts, traversed, module) => {
  ...
  if (traversed[id] && traversed[id].stamp === module.stamp) {
    ...
    if (compareSemver(file.version, traversed[id].file.version)) {
      ...
      str += ` use ${chalk.blue(file.version)} over ${version}`
    } else if (traversed[id].file.version !== file.version) {
      str += ` use ${version} over ${chalk.blue(file.version)}`
    } else {
      str += ` use ${version}`
    }
