...
const JSXElement = (node, code, replace) => {
  if (node.type === 'JSXElement') {
    ...
    if (expression) {
      ...
      // find element
      if (inSwitch(node)) {
        // add the switcher there
        // get an id for the child or somethign
        elem = node.elem = { _children_: [], _attributes_: {}, _inSwitch_: expression }
        // elem._inSwitch_ = true
        // here it can just add _switches_ or something
        // also need to decide what is self -- leave for now
        switcher(expression, elem, node, code)
        console.log('ok its a switch!')
      } else {
