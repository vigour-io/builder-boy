...
const state = (node, code, replace, keys) => {
  ...
  walker(node, node => {
    ...
    if (node.type === 'MemberExpression' && node.object && (key = keys.includes(node.object.name) && node.object.name)) {
      ...
      while (p && p.type !== 'JSXExpressionContainer' && p.type !== 'JSXElement') {
        if (p.type === 'ArrowFunctionExpression' || p.type === 'FunctionExpression') {
          ...
          if (p.object && p.object.property) {
            ...
            if ((p.callee.property.name === 'root' || p.callee.property.name === 'parent') && !p.arguments.length) {
              // only when no ARGUMENTS
              arr.push({ name: p.callee.property.name, node: p.callee.property, key, brackets: true })
            } else {
      ...
      states.push(arr)
    } else if (node.type === 'JSXExpressionContainer' || node.type === 'JSXElement') {
      return true
    }
  })
  console.log(states)
    ...
    if (safe && !$any) {
      ...
      parseState.forEach((parseStateIterate, index) => {
        if (parseStateIterate.length) {
          ...
          showcode(code, parseStateIterate[0].node.parent.start - trim, end + s + trim)
