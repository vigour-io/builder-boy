...
// const weakMerge = (elem) => {
//   var _mergit = elem
//   for (let key in _mergit) {
//     if (!elem[key]) {
//       elem[key] = _mergit[key]
//     } else if (typeof elem[key] === 'object' && typeof _mergit[key] === 'object') {
//       weakMerge(elem[key], _mergit[key])
//     }
//   }
// }
...
const parseCode = (elem, replace, code, result = []) => {
  ...
  if (elem._children_) {
    elem._children_.forEach((val, key) => {
      ...
      if (val.tag && (val.tag === `'body'` || val.tag === `'head'` || val.tag === `'title'`)) {
        if (!val._attributes_) val._attributes_ = {}
        if (!val._attributes_.key) val._attributes_.key = val.tag
      }
