...
const singleNodeStateExpression = (node, code, replace, expressionCode, trim = 2) => {
  ...
  if (parseState.length) {
    ...
    parseState.forEach(val => {
      ...
      if (last.isFn) {
        if (last.name === 'compute') {
          // need to support more e.g. keys / length etc etc / get
          mergeWithPath($, val.slice(0, -1), { val: str('shallow') })
          str$ = str(val.slice(0, -1).map(val => val.name).join('.'))
        } else if (last.name === 'map') {
          $any = true
          str$ = str(val.slice(0, -1).map(val => val.name).join('.') + '.$any')
          // mergeWithPath($, val.slice(0, -1), { val: str('shallow') })
        }
      }
    })
    ...
    if (parseState.length === 1) {
      ...
      if ($any) {
        ...
        // need to replace the whole map thing
      } else {
        localReplace.push({
          start,
          end,
    ...
    return { replace: localReplace, $: str$, $any }
  }
}
  ...
  if (node.expression && node.expression.type === 'ObjectExpression') {
    ...
    if (result) {
      if (typeof expressionCode !== 'object') {
        expressionCode = compile(expressionCode, result.replace).join('')
        return { code: parseFn(node, expressionCode), $: result.$ }
      } else {
        return { code: expressionCode, $: result.$, $any: result.$any }
      }
    }
  }
}
    ...
    if (
      ...
      if (subscription) {
        ...
        if (subscription.$any) {
          ...
          elem.$ = subscription.$ // merge if nessecary
          // if ! props
          if (!elem.props) elem.props = {}
          elem.props.default = subscription.code
...
const expressionCompilation = (replace, code, elem, key, result) => {
  ...
  var start, end
  if (elem[key].$any) {
    console.log('REPLACE', elem[key])
    showcode(code, elem[key].arguments[0].body)
