...
const parseJSX = require('../../jsx-struct/ast')
...
const parseIt = (file, ast, computed) => new Promise((resolve, reject) => {
  if (!computed) {
    computed = file.code.compute()
  }
  ...
  const parse = node => {
    ...
    if (node.type === 'JSXElement') {
      parseJSX(file, ast)
        .then(({ result }) => {
          return parseIt(file, false, result)
        })
        .then(resolve)
        .catch(err => {
