...
const findUaName = (ua, node) => {
  for (let i = 0, len = ua.length; i < len; i++) {
    if (ua[i].type === 'imports' || ua[i].exports && ua[i].exports.default) {
      if (node.type === 'MemberExpression') {
        if (ua[i].default === node.name) {
      ...
      for (let j = 0, len = ua[i].objectPattern.length; j < len; j++) {
        console.log(ua[i].objectPattern[j])
        if (ua[i].objectPattern[j].val === node.name) {
          return node.name
        } else if (ua[i].objectPattern[j].key && ua[i].objectPattern[j].val.name === node.name) {
          return ua[i].objectPattern[j].key.name
        }
      }
    }
  }
