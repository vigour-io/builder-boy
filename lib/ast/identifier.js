...
const setStore = (node, replaceImports, insertId, store, shorthand, computed) => {
  ...
  if (
    ...
    if (!hasLocalVar(node)) {
      if (node.name === 'parent' && store[node.name]) {
        // showcode(node, false, compute)
        logTypeChain(node)
      }
...
const identifier = (node, replaceImports, insertId, store, shorthand, computed) => {
  if (node.name && node.type === 'Identifier') {
    if (parent(node, blockVar)) { // || parent(node, blockInlinevar)
      if (node.parent.id.name !== node.name) {
        setStore(node, replaceImports, insertId, store, shorthand, computed)
      } else {
        const fn = getFn(node)
        if (fn) {
